...
class C_Item
{
	...
	var string name;
	var string nameID;
	var int hp;		//Zatocheno
	var int hp_max;		//Mozhno tochit'
	var int mainflag;
	var int flags;
	var int weight;	//Otravleno ili net
	var int value;
	var int damagetype;
	var int damageTotal;
	var int damage[DAM_INDEX_MAX];
	var int wear;
	var int protection[PROT_INDEX_MAX];
	var int nutrition;	//Mozhno otravit'
	var int cond_atr[3];
...
class C_STATUSBAR
{
	var int			posx;	//Polozheniye polosy po osi X (maks. znacheniye 864(8192/9.5) )
	var int			posy;	//Polozheniye polosy po osi Y (maks. znacheniye 864(8192/9.5) )
	var int			sizex;	//Razmer po osi X(maks. znacheniye 864)
	var int			sizey;	//Razmer po osi Y(maks. znacheniye 864)
	var	int 		value;
	var	int 		maxrange;
	var	STRING 		backtex;
	var	STRING 		bartex;
...
{
	posx		= 0;	//Polozheniye polosy po osi X (maks. znacheniye 864(8192/9.5) )
	posy		= 0;	//Polozheniye polosy po osi Y (maks. znacheniye 864(8192/9.5) )
	sizex		= 0;	//Razmer po osi X(maks. znacheniye 864)
	sizey		= 0;	//Razmer po osi Y(maks. znacheniye 864)
	value		= 0;	
	maxrange	= 0;
	backtex		= "";
	bartex		= "";
...
{
	posy = 1; //Pomestit' satus bar pod bar khp tseli
	value		= 1;
	maxrange	= 10;
