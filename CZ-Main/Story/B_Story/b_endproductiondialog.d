...
func void b_endproductiondialog()
{
	...
	if((PLAYER_MOBSI_PRODUCTION == MOBSI_PRAYIDOL) && (SOUL_PRAY_AFTER == TRUE) && (SOUL_PRAY_AFTER_END == FALSE))
	{
		...
		if(GUARDIAN_RING == TRUE)
		{
			...
			Wld_PlayEffect("VOB_MAGICBURN",hero,hero,0,0,0,FALSE);
			concatText = "Síla prstenu Strážců tě ochránila před hněvem Temného boha!";
			AI_Print(concatText);
		...
		else
		{
			...
			B_LogEntry(TOPIC_BELIARCURSE,"Naivně jsem nepoužil prsten. Proti Temnému bohu jsem byl zcela bezmocný. Nyní jsem prokletý, těžko říci jaké to bude mít následky ale jedno je jisté, je třeba se prokletí zbavit!");
			Wld_PlayEffect("spellFX_BELIARSRAGE",hero,hero,0,0,0,FALSE);
			Snd_Play("DEM_Warn");
			Wld_PlayEffect("VOB_MAGICBURN",hero,hero,0,0,0,FALSE);
			concatText = "Temný bůh tě proklel!";
			AI_Print(concatText);
			Snd_Play("MFX_FEAR_CAST");
