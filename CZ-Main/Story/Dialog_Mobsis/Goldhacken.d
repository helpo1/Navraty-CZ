	...
	else if(ATR_STAMINA[0] >= staminaforpickgold)
	{
		...
		if(B_GoldMob_Bestimmung() == TRUE)
		{
			...
			if(Hero_HackChance >= CurrentChance)
			{
				...
				{
					AI_PrintClr("Kousky zlata lítají všude kolem...",245,247,225);
		...
		{
			AI_PrintClr("Tady už nic není!",177,58,17);
		};
	}
	else
	{
...
instance PC_Goldhacken_Addon_TSchlag(C_Info)
{
	...
	permanent = TRUE;
	description = "Těžit zlato (silný úder)";
};
...
func void PC_Goldhacken_Addon_TSchlag_Info()
{
	...
	if(ATR_STAMINA[0] >= staminaforpickgold)
	{
		...
		if(Hero_HackChance >= 85)
		{
			...
			concatText = ConcatStrings(concatText,"x Zlatý nuget");
			concatText = ConcatStrings(concatText," (celkem: ");
			concatText = ConcatStrings(concatText,IntToString(Npc_HasItems(hero,ItMi_Addon_GoldNugget)));
			concatText = ConcatStrings(concatText,")");
			AI_PrintClr(concatText,83,152,48);
...
instance PC_Goldhacken_Osair(C_Info)
{
	...
	permanent = TRUE;
	description = "Odštípnout kus horniny";
};
...
func void PC_Goldhacken_Osair_Info()
{
	...
	Snd_Play("ORE_HACK");
	B_LogEntry(TOPIC_PW_GOLDSHAHT,"Mám vzorek zlatonosné žíly z dolu v podhorské jeskyni! Je čas vrátit se za Osairem.");
};
...
instance PC_Goldhacken_Crystall(C_Info)
{
	...
	permanent = TRUE;
	description = "Odlomit kus krystalu";
};
...
func void PC_Goldhacken_Crystall_Info()
{
	...
	if(AllCrystal <= 6)
	{
		...
		if(ATR_STAMINA[0] >= staminaforpickgold)
		{
			...
			AI_PrintClr("Získal jsi krystal...",83,152,48);
	...
	{
		AI_PrintClr("Tady nic není!",177,58,17);
