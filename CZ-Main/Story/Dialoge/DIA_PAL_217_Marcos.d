...
{
	AI_Output(self,other,"DIA_Marcos_Hallo_04_00");	//Stůj! Ve jménu Innose! Jsem královský paladin! Řekni kdo jsi a co tu chceš! A pravdu!

	if(other.guild == GIL_KDF)
	{
		AI_Output(other,self,"DIA_Marcos_Hallo_15_01");	//Innosův vyvolený mluví vždy pravdu.
		AI_Output(self,other,"DIA_Marcos_Hallo_04_02");	//Odpust ctihodný mágu. Neuvědomil jsem si s kým mluvím!
...
instance DIA_Marcos_Hagen(C_Info)
{
	...
	permanent = FALSE;
	description = "Mám lordu přinést důkaz o přítomnosti draků.";
};
...
{
	AI_Output(other,self,"DIA_Marcos_Hagen_15_00");	//Mám lordu přinést důkaz o přítomnosti draků.
	...
	AI_Output(self,other,"DIA_Marcos_Hagen_04_04");	//Musí se dozvědět, že jsi na misi pro lorda Hagena. Myslím, že ti dá ten tvů důkaz...
};
...
func void DIA_Marcos_Garond_Info()
{
	...
	AI_Output(self,other,"DIA_Marcos_Garond_04_02");	//Zkoušeli jsem se dostat do pevnosti, ale nakonec jsem přežil jen já.
	...
	B_LogEntry(TOPIC_ScoutMine,"Paladin Markus ochraňuje čtyři bedny rudy v malém údolíčku vedle pevnosti.");
	Log_CreateTopic(Topic_MarcosJungs,LOG_MISSION);
	Log_SetTopicStatus(Topic_MarcosJungs,LOG_Running);
	B_LogEntry_Quiet(Topic_MarcosJungs,"Markus chce, aby mu Garond poslal posily.");
...
func void DIA_Marcos_Perm_Info()
{
	...
	{
		AI_Output(self,other,"DIA_Marcos_Perm_04_02");	//Věřím a doufám, že ji Garond pošle.
	}
