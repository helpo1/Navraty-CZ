...
func void dia_SEK_156_SLAVEOBSSEK_hallo_info()
{
	...
	{
		AI_Output(self,other,"DIA_SEK_156_SLAVEOBSSEK_HALLO_01_01");	//Jestli se odsud nedostanu, tak brzo zemřu!
		FindMistake = TRUE;
		AI_StopProcessInfos(self);
	}
	else if((MIS_RebelSlave == LOG_Running) && (PW_BetrayFollowMe == FALSE) && (BetrayFound == TRUE))
	{
		...
		AI_Output(other,self,"DIA_SEK_156_SLAVEOBSSEK_HALLO_01_04");	//Chtěl by ti osobně poděkovat za pomoc při chycení spiklence. 
	...
	else if((MIS_RebelSlave == LOG_Running) && (FindMistake == TRUE) && (KnowWhoRebels == FALSE))
	{
		...
		AI_Output(other,self,"DIA_SEK_156_SLAVEOBSSEK_HALLO_01_11");	//Jinak se postarám aby se o tebe osobně zajímala Osairova ochranka.
		...
		B_LogEntry(TOPIC_RebelSlave,"Myslím, že jsem na stopě. Jeden ze sloužících prozradil, že otrok Marius připravuje útěk a podněcuje ke vzpouře. Musím nejdřív jeho tvrzení prověřit, než podám Osairovi hlášení.");
		KnowWhoRebels = TRUE;
		MariusIns = TRUE;
		AI_StopProcessInfos(self);
