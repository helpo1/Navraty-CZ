...
func int B_DragonKillCounter(var C_Npc current_dragon)
{
	...
	if(Hlp_GetInstanceID(current_dragon) == Hlp_GetInstanceID(Ravd))
	{
		if(RavenIsDead == FALSE)
		{
			PlayVideoEx("EXTRO_RAVEN.BIK",TRUE,FALSE);
			RavenIsDead = TRUE;
			B_LogEntry_Quiet(TOPIC_AdanosCrone,"Zdá se, že jsem zapudil veliké zlo zde v chrámu! Raven opustil Beliarovu náruč. Nastal čas vrátit se zpět a povědět vše Saturasovi.");
	...
	if(current_dragon.guild == GIL_DRAGON)
	{
		...
		{
			BLKDRAGNISDEAD = TRUE;
			B_LogEntry(TOPIC_URNAZULRAGE,"Nastal čas radovánek, když velký zlý drak Azgalor padl! Nyní bych se měl nejspíše uchýlit zpět k Ur-Thrallovi a přinést mu dračí srdce.");
