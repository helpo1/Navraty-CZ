...
instance Spell_OrcFireball(C_Spell_Proto)
{
	time_per_mana = 0;
	damage_per_level = SPL_Damage_OrcFireball;
	damagetype = DAM_FIRE;
};

func int Spell_Logic_OrcFireball(var int manaInvested)
{
	if((self.guild == GIL_FIREGOLEM) || (self.aivar[AIV_MM_REAL_ID] == ID_DRAGON_FIRE))
	{
		AI_PrintClr("To nebude fungovat...",177,58,17);
	...
	else if(self.attribute[ATR_MANA] >= SPL_Cost_OrcFireball)
	{
		return SPL_SENDCAST;
	}
	else
...
func void Spell_Cast_OrcFireball()
{
	if(Npc_IsPlayer(self) && (PLAYERISTRANSFER == TRUE) && (PLAYERISTRANSFERDONE == FALSE))
	{
		b_transferback(self);
	...
	{
		self.attribute[ATR_MANA] = self.attribute[ATR_MANA] - SPL_Cost_OrcFireball;
	};
	if(Npc_IsPlayer(self) && (MIS_RUNEMAGICNOTWORK == LOG_Running) && (TESTRUNEME == FALSE) && !Npc_GetActiveSpellIsScroll(self))
	{
		if((FIREMAGERUNESNOT == TRUE) || (WATERMAGERUNESNOT == TRUE) || (GURUMAGERUNESNOT == TRUE) || (PALADINRUNESNOT == TRUE))
...
instance Spell_RapidFirebolt(C_Spell_Proto)
{
	time_per_mana = 0;
	damage_per_level = SPL_Damage_RapidFirebolt;
...
func void Spell_Cast_Explosion()
{
	...
	{
		self.attribute[ATR_MANA] = self.attribute[ATR_MANA] - SPL_Cost_Explosion;
	};
	if(Npc_IsPlayer(self) && (MIS_RUNEMAGICNOTWORK == LOG_Running) && (TESTRUNEME == FALSE) && !Npc_GetActiveSpellIsScroll(self))
	{
		if((FIREMAGERUNESNOT == TRUE) || (WATERMAGERUNESNOT == TRUE) || (GURUMAGERUNESNOT == TRUE) || (PALADINRUNESNOT == TRUE))
