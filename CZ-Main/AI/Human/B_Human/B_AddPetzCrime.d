...
func void B_AddPetzCrime(var C_Npc slf,var int crime)
{
	if(C_NpcBelongsToOldCamp(slf))
	{
		if(crime == CRIME_MURDER)
		{
			...
			AI_PrintClr("Tvé vraždění nezůstalo bez povšimnutí!",177,58,17);
		};
		if(crime == CRIME_THEFT)
		{
			...
			INNOSCRIMECOUNT = INNOSCRIMECOUNT + 1;
			AI_PrintClr("Tvé kradení nezůstalo bez povšimnutí!",223, 121, 0);
		};
		if(crime == CRIME_ATTACK)
		{
			...
			INNOSCRIMECOUNT = INNOSCRIMECOUNT + 1;
			AI_PrintClr("Tvé útoky nezůstaly bez povšimnutí!",223, 121, 0);
		};
	...
	if(C_NpcBelongsToCity(slf))
	{
		...
		if(crime == CRIME_SHEEPKILLER)
		{
			...
			AI_PrintClr("Tvé zabíjení ovcí nezůstalo bez povšimnutí!",223, 121, 0);
		};
	};
