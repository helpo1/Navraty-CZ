		...
		if((TrollEatMeat == TRUE) && (PoisonResult == FALSE))
		{
			...
			else if(TrollCountStep == 10)
			{
				if(PoisonDoneFull == TRUE)
				{
					...
					B_LogEntry(TOPIC_PW_PoisonNrozas,"Trol očichal návnadu, ale nesnědl ji. Zdá se, že jsem to s množstvím jedu přehnal...");
					PoisonResult = TRUE;
				};
			}
			...
			else if(TrollCountStep == 15)
			{
				if(PoisonDoneHalf == TRUE)
				{
					...
					B_LogEntry(TOPIC_PW_PoisonNrozas,"Trol snědl návnadu a okamžitě zemřel! Zdá se, že se mi to povedlo...");
				...
				{
					Wld_SendTrigger("EVT_TROLL_POISON");
					B_LogEntry(TOPIC_PW_PoisonNrozas,"Trol spolkl návnadu, ale jed na něho nepůsobil. Vypadá to tak, že jsem použil příliš malé množství jedu... Musím si rychle promluvit s Gonzalesem o tom, co bych teď měl udělat!");
					PoisonResult = TRUE;
				};
			};
