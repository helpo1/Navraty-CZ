	...
	if(Hlp_GetInstanceID(self) == Hlp_GetInstanceID(Skeleton_Shadow_Priest_Quardimon))
	{
		if(CrownIsUp == FALSE)
		{
			...
			if((MIS_Qvardemon == LOG_Running) && (FTDamageQvardemon == FALSE))
			{
				...
				B_LogEntry(Topic_JarCurse,"Vypadá to tak, že tenhle kněz bude ještě tvrdý oříšek! Obyčejné zbraně ho vůbec nezraňují. Co teď budu dělat?");
			};
		}
		else
		{
	...
	if(Npc_IsPlayer(other) && (self.guild == GIL_ORC) && (GUARDIANSISDEFEATED == FALSE) && (KAPITELORCATC == TRUE))
	{
		if(Npc_GetDistToWP(other,"NW_CITY_MERCHANT_PATH_11") <= 32000)
		{
			...
			if(FIRSTSTRIKEME == FALSE)
			{
				...
				{
					B_LogEntry(TOPIC_STRANGETHINGS,"Už naprosto přestávám rozumět tomu, co se tady děje! Když jsem se pokusil napadnout jednoho ze skřetů, zasáhl mě opravdu nepříjemný blesk, který mě téměř zabil. Tyhle bestie zřejmě chrání nějaká magie. Zdá se, že teď musím být opatrnější...");
				}
				else if(MIS_STRANGETHINGS == FALSE)
				{
					...
					B_LogEntry(TOPIC_STRANGETHINGS,"Děje se tu něco podivného! Když jsem se pokusil napadnout jednoho ze skřetů, zasáhl mě opravdu nepříjemný blesk, který mě téměř zabil. Tyhle bestie zřejmě chrání nějaká magie. Zdá se, že teď musím být opatrnější...");
				};
			};
		}
