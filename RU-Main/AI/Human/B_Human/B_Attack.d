	...
	if((Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(VLK_480_Fellan)) && (FELLANGORFIRSTMEET == TRUE))
	{
		if(FELLANGORTHIRDMEET == TRUE)
		{
			if(FELLANGORSAVE == FALSE)
			{
				if(MIS_BELIARHELPME == LOG_Running)
				{
					B_LogEntry(TOPIC_BELIARHELPME,"Я уничтожил Феллангора! Пора вернуться к Зигосу и получить обещанную награду.");
				};
			...
			else
			{
				if(MIS_BELIARHELPME == LOG_Running)
				{
					B_LogEntry(TOPIC_BELIARHELPME,"Я отпустил Феллангора! Не думаю, что мне стоит рассчитывать теперь на благодарность Зигоса.");
				};
				Wld_StopEffect("DEMENTOR_FX");
				Npc_ExchangeRoutine(slf,"TOT");
