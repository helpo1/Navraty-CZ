	...
	if((Npc_GetStateTime(self) > 7) && (self.aivar[AIV_TAPOSITION] == ISINPOS))
	{
		...
		if((Hlp_GetInstanceID(self) == Hlp_GetInstanceID(Ass_128_Nrozas)) && (NrozasAway == FALSE))
		{
			...
			if((MIS_NrozasPoisonGift == LOG_Success) && (HaniarGetGift == FALSE) && (Npc_GetStateTime(self) >= 15))
			{
				...
				B_LogEntry(TOPIC_MasiafStory,"Черт! Вино, которое мне дал Осаир, было отравлено...Теперь меня обвинят в убийстве Нрозаса. Остается только драться!");
				return LOOP_END;
			}
			else if((MIS_NrozasPoisonGift == LOG_Success) && (HaniarGetGift == TRUE) && (Npc_GetStateTime(self) >= 15))
			{
				...
				B_LogEntry(TOPIC_MasiafStory,"Нрозас выпил вина и исчез...Видимо, план Ханиара начал работать! Но мне надо во чтобы то ни стало узнать, о каком внутреннем святилище братства говорил Ханиар.");
