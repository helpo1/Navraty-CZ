...
func void ZS_MM_ThreatenEnemy()
{
	...
	if(Npc_IsPlayer(other) && (Hlp_GetInstanceID(self) == Hlp_GetInstanceID(TROLL_YOUNG_PW)) && (TrollSeeMe == FALSE))
	{
		if(MIS_PW_PoisonNrozas == LOG_Running)
		{
			...
			{
				B_LogEntry(TOPIC_PW_PoisonNrozas,"Черт, я спугнул тролля! Навряд ли мне теперь представится шанс отравить наживку и испытать яд Нрозаса...");
			}
			else
			{
				B_LogEntry(TOPIC_PW_PoisonNrozas,"Черт, я спугнул тролля, однако успел отравить наживку! Может быть, эта тварь все таки решит отведать моего лакомства...");
			};
		};
	};
};
