...
func void DIA_Addon_Martin_PreTrade_Info()
{
	...
	AI_Output(self,other,"DIA_Addon_Martin_PreTrade_07_01");	//Хммм...(задумчиво) Скажем так, если ты компенсируешь мне те неприятности, которые у меня возникнут, когда пропажи обнаружатся, то мы можем договориться.
...
instance DIA_Addon_Martin_Farim(C_Info)
{
	...
	description = "У рыбака Фарима проблемы с ополчением.";
};
...
instance DIA_Addon_Martin_Coragon(C_Info)
{
	...
	permanent = FALSE;
	description = "Ты можешь мне помочь в одном дельце?";
};
...
{
	AI_Output(other,self,"DIA_Addon_Martin_Coragon_01_00");	//Ты можешь мне помочь в одном дельце?
	AI_Output(self,other,"DIA_Addon_Martin_Coragon_01_01");	//Смотря в чем оно заключается.
	AI_Output(other,self,"DIA_Addon_Martin_Coragon_01_02");	//У трактирщика Корагона большие проблемы из-за того, что Вирт по приказу паладинов раздает бесплатное пиво.
	AI_Output(other,self,"DIA_Addon_Martin_Coragon_01_03");	//Он теряет клиентов, а значит, и свой доход. Смекаешь, к чему я клоню?
	AI_Output(self,other,"DIA_Addon_Martin_Coragon_01_04");	//(деловито) Ну, допустим. И что?
	AI_Output(other,self,"DIA_Addon_Martin_Coragon_01_05");	//Ты можешь как-то повлиять на эту ситуацию?
	AI_Output(self,other,"DIA_Addon_Martin_Coragon_01_06");	//Приказ паладинов может отменить только лорд Хаген, или же лорд Андрэ.
	AI_Output(self,other,"DIA_Addon_Martin_Coragon_01_07");	//Мне это не под силу.
	AI_Output(other,self,"DIA_Addon_Martin_Coragon_01_08");	//Неужели ничего нельзя сделать?
	AI_Output(self,other,"DIA_Addon_Martin_Coragon_01_09");	//(ехидно) Ну, все зависит от того, что ты можешь мне предложить взамен.
	AI_Output(other,self,"DIA_Addon_Martin_Coragon_01_10");	//Две бочки превосходнейшего пива тебя устроят?
	AI_Output(self,other,"DIA_Addon_Martin_Coragon_01_11");	//(задумчиво) Да, это весьма интересное предложение.
	AI_Output(self,other,"DIA_Addon_Martin_Coragon_01_12");	//Значит, говоришь, это Вирт раздает пиво на городской площади?
	AI_Output(other,self,"DIA_Addon_Martin_Coragon_01_13");	//Он самый.
	AI_Output(self,other,"DIA_Addon_Martin_Coragon_01_14");	//Хорошо. Дай мне некоторое время - примерно один день, и я улажу проблему Корагона.
	AI_Output(self,other,"DIA_Addon_Martin_Coragon_01_15");	//А потом я жду мое пиво! Не забудь о нем.
	AI_Output(other,self,"DIA_Addon_Martin_Coragon_01_16");	//Конечно.
	MartinSpeakUpDay = Wld_GetDay();
	B_LogEntry(TOPIC_CoragonFixBeer,"Мартин взялся уладить проблему Корагона в течении дня за две бочки пива. Интересно, как он это сделает?");
};