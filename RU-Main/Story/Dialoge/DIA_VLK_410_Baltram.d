...
func void DIA_Baltram_Trick_Info()
{
	...
	AI_Output(self,other,"DIA_Baltram_Trick_01_04");	//Я дам тебе пятьдесят золотых монет.
	...
	B_LogEntry_Quiet(TOPIC_Nagur,"Бальтрам нанял меня в качестве мальчика на побегушках. Теперь, я должен доставить ему посылку с фермы Акила.");
	Log_CreateTopic(TOPIC_Baltram,LOG_MISSION);
	Log_SetTopicStatus(TOPIC_Baltram,LOG_Running);
	B_LogEntry(TOPIC_Baltram,"Если я принесу Бальтраму его посылку, он заплатит мне полсотни золотых монет.");
};
...
{
	AI_Output(other,self,"DIA_Addon_Baltram_Skip_15_00");	//Значит, ты торгуешь с пиратами?
	AI_Output(self,other,"DIA_Addon_Baltram_Skip_01_01");	//Что?...(испуганно) Кто тебе это сказал?
...
{
	AI_Output(other,self,"DIA_Addon_Baltram_Skip_Ich_15_00");	//Я бы мог совершить сделку вместо тебя.
...
func void DIA_Addon_Baltram_SkipsRum_Info()
{
	...
	AI_Output(self,other,"DIA_Addon_Baltram_SkipsRum_01_01");	//Дороговато получается... Что он о себе возомнил? Ладно, давай их сюда.
...
func void DIA_Baltram_Lieferung_Info()
{
	...
	AI_Output(self,other,"DIA_Baltram_Lieferung_01_01");	//Превосходно! Теперь я опять смогу торговать свежими продуктами. Вот твои пятьдесят золотых монет.
...
func void DIA_Baltram_LetUsTrade_Info()
{
	...
	AI_Output(self,other,"DIA_Baltram_LetUsTrade_01_01");	//Послушай, если ты принесешь мне десять окороков и десять бутылок вина, тогда я опять готов иметь с тобой дело.
	...
	B_LogEntry(TOPIC_BaltramTradeAgain,"Если я принесу Бальтраму десять окороков и десять бутылок вина, тогда он снова будет иметь со мной дело.");
};
...
func void DIA_Baltram_HaveYourWarez_Info()
{
	...
	{
		AI_Output(self,other,"DIA_Baltram_HaveYourWarez_01_02");	//Мы договорились на десять окороков! Возвращайся когда добудешь их.
	...
	{
		AI_Output(self,other,"DIA_Baltram_HaveYourWarez_01_03");	//А что насчет десяти бутылок вина? Ты что, забыл про них?
	...
	if((BaltramEnoughBacon == TRUE) && (BaltramEnoughWine == TRUE))
	{
		...
		B_LogEntry(TOPIC_BaltramTradeAgain,"Бальтрам теперь вновь будет со мной торговать.");
