...
func void DIA_Pedro_DI_YOU_Info()
{
	...
	{
		AI_Output(other,self,"DIA_Pedro_DI_YOU_15_0A");	//Как ты мог пойти на такое! Даже я, слуга Аданоса, не могу поверить в это!
	};
	if(hero.guild == GIL_KDM)
	{
		AI_Output(other,self,"DIA_Pedro_DI_YOU_15_0B");	//Хоть я и служу Темному богу, но предательство ни у кого не в чести!
	};
	if((hero.guild == GIL_TPL) || (hero.guild == GIL_GUR))
	{
		AI_Output(other,self,"DIA_Pedro_DI_YOU_15_0C");	//Конечно, Братству наплевать на эти разборки богов. Но за свой поступок ты заслуживаешь только одного!
	};
	AI_Output(other,self,"DIA_Pedro_DI_YOU_15_04");	//Я должен просто тебя убить.
	AI_Output(self,other,"DIA_Pedro_DI_YOU_09_05");	//Погоди. Прошу тебя. Они околдовали меня. Никогда бы по своей воле я не предал бы монастырь Инноса.
	AI_Output(self,other,"DIA_Pedro_DI_YOU_09_06");	//Ты даже не знаешь, что мне пришлось пережить в последние дни. Они засунули мне в голову этот голос. Я не мог ему противостоять.
	AI_Output(self,other,"DIA_Pedro_DI_YOU_09_07");	//Вождь орков пытал меня дни напролет. Он постоянно меня бил. Прости меня. Это не моя вина. Ты должен поверить мне.
...
{
	AI_Output(other,self,"DIA_Pedro_DI_YOU_Serpentes_15_00");	//Серпентес дал мне поручение убить тебя, как правило, я выполняю свои поручения.
	AI_Output(self,other,"DIA_Pedro_DI_YOU_Serpentes_09_01");	//(Жалобно) Пожалуйста, нет. Ты не сделаешь этого. Я могу еще тебе пригодиться. Ты знаешь это.
};
...
func void DIA_Pedro_DI_YOU_FollowShip()
{
	...
	if(MIS_Gorax_KillPedro == LOG_Running)
	{
		B_LogEntry(Topic_Gorax_KillPedro,"Я решил сохранить жизнь Педро. Не знаю, правильно ли я поступил?!");
	};
};
...
func void DIA_Pedro_DI_ArrivedAtShip_OrkOberst()
{
	...
	{
		Info_AddChoice(DIA_Pedro_DI_ArrivedAtShip,"Вождь орков мертв.",DIA_Pedro_DI_ArrivedAtShip_OberstTot);
	};
};
...
{
	AI_Output(other,self,"DIA_Pedro_DI_ArrivedAtShip_Back_15_00");	//Я достаточно услышал. Поищи себе место на моем корабле. Мне еще нужно кое-что сделать.
	AI_Output(self,other,"DIA_Pedro_DI_ArrivedAtShip_Back_09_01");	//Пусть сила Инноса защитит тебя. Я буду молиться за тебя.
	...
	if((Vatras_IsOnBoard != LOG_SUCCESS) && (MIS_Gorax_KillPedro == LOG_Running))
	{
		...
		B_LogEntry(Topic_Gorax_KillPedro,"Педро пока останется на нашем корабле. Думаю, я не вправе решать его судьбу самостоятельно. Пускай его судят сами маги Огня.");
