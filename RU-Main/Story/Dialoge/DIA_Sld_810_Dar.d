...
func void DIA_Dar_Hallo_Ja()
{
	...
	AI_Output(self,other,"DIA_Dar_Hallo_Ja_03_03");	//У меня свои источники...(усмехается)
	Info_ClearChoices(DIA_Dar_Hallo);
};
...
func void DIA_Dar_PERM_Info()
{
	...
	{
		AI_Output(self,other,"DIA_Dar_PERM_03_01");	//Иногда я позволяю всяким мстительным болотным наркоманам задать мне взбучку...
		Dar_einmal = TRUE;
	}
	else
	{
...
{
	AI_Output(other,self,"DIA_Dar_DuDieb_15_00");	//Сайфер сказал мне, что кто-то украл у него тюк болотной травы.
...
{
	AI_Output(self,other,"DIA_Dar_Kameradenschwein_03_00");	//Трепач! Ты сказал Сайферу, что я взял его траву!
...
func void DIA_Dar_ORCRING_Info()
{
	...
	Info_AddChoice(DIA_Dar_ORCRING,"Мне это не интересно.",DIA_Dar_ORCRING_no);
	Info_AddChoice(DIA_Dar_ORCRING,"Крутой? ТЫ?",DIA_Dar_ORCRING_necken);
	...
	{
		Info_AddChoice(DIA_Dar_ORCRING,"Ну и как это должно выглядеть?",DIA_Dar_ORCRING_wie);
	};
};
...
func void DIA_Dar_BRINGORCELITERING_geld()
{
	...
	if((hero.guild == GIL_SLD) || (hero.guild == GIL_DJG))
	{
		AI_Output(self,other,"DIA_Dar_BRINGORCELITERING_geld_03_01");	//Ммм...шестьдесят золотых монет?
		AI_Output(other,self,"DIA_Dar_BRINGORCELITERING_geld_15_02");	//Что?
	};

	AI_Output(self,other,"DIA_Dar_BRINGORCELITERING_geld_03_03");	//Ладно! Я дам тебе сто двадцать монет.
	...
	Info_ClearChoices(DIA_Dar_BRINGORCELITERING);
	Info_AddChoice(DIA_Dar_BRINGORCELITERING,"Мне этого мало.",DIA_Dar_BRINGORCELITERING_geld_no);
...
func void DIA_Dar_BRINGORCELITERING_was()
{
	...
	Info_ClearChoices(DIA_Dar_BRINGORCELITERING);
	Info_AddChoice(DIA_Dar_BRINGORCELITERING,"Давай мне деньги.",DIA_Dar_BRINGORCELITERING_geld);
