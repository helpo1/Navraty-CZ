...
func void DIA_Canthar_Hallo_Info()
{
	...
	if((Npc_HasEquippedArmor(other) == FALSE) || Hlp_IsItem(itm,ITAR_Her_M) || Hlp_IsItem(itm,ITAR_Her_N) || Hlp_IsItem(itm,ITAR_Prisonel) || Hlp_IsItem(itm,ITAR_Prisoner) || Hlp_IsItem(itm,itar_sekbed))
	{
		...
		AI_Output(self,other,"DIA_Canthar_Hallo_09_03");	//Хммм...(презрительно) Ты похож на беглеца! А все беглецы направляются в город.
		AI_Output(self,other,"DIA_Canthar_Hallo_09_04");	//Возможно, ты даже беглый каторжник из колонии.
		AI_Output(self,other,"DIA_Canthar_Hallo_09_05");	//Меня не волнует, откуда ты пришел! Но мне кажется, у меня есть интересное предложение для тебя.
		Canthar_GotMe = TRUE;
	}
...
func void DIA_Canthar_Hallo_NoBauer()
{
	...
	AI_Output(self,other,"DIA_Canthar_Hallo_NoBauer_09_02");	//Если я ни в чем не ошибаюсь, у меня есть интересное предложение для тебя.
	Canthar_GotMe = TRUE;
	Info_ClearChoices(DIA_Canthar_Hallo);
};
...
func void DIA_Canthar_WhatOffer_Info()
{
	...
	if(Canthar_GotMe == TRUE)
	{
		...
		AI_Output(self,other,"DIA_Canthar_WhatOffer_09_04");	//У меня есть вот этот клочок бумаги - с королевской печатью и подписью главы города. Пропуск!
		if(Npc_HasEquippedArmor(other) == FALSE)
		{
...
func void DIA_Canthar_PAYPRICEINCITY_Ok()
{
	...
	B_LogEntry(TOPIC_Canthar,"Я должен подложить письмо торговке Саре, которое даст повод подозревать ее в том, что она продает оружие Онару. Затем я должен пойти к лорду Андрэ и сдать ее ополчению. На все это Кантар дал мне два дня...");
	Info_ClearChoices(DIA_Canthar_PAYPRICEINCITY);
};
...
instance DIA_Canthar_SARAHERLEDIGT(C_Info)
{
	...
	permanent = TRUE;
	description = "У тебя нет работы для меня?";
};
...
{
	AI_Output(other,self,"DIA_Canthar_SARAHERLEDIGT_09_00");	//У тебя нет работы для меня?
...
func void DIA_Canthar_Success_Info()
{
	...
	if(Canthar_Gefallen == FALSE)
	{
		...
		AI_Output(self,other,"DIA_Canthar_Success_09_03");	//Правильно! Вот, это оружие - произведение оружейного искусства...(посмеиваясь)
...
func void DIA_Canthar_Pay_Ja()
{
	...
	AI_Output(self,other,"DIA_Canthar_Pay_Ja_09_01");	//Хорошо...(ухмыляется) А теперь я позабочусь, чтобы твоя репутация среди торговцев выросла опять.
...
{
	B_GivePlayerXP(50);
	AI_Output(other,self,"DIA_Canthar_KillNow_01_00");	//Боюсь, у меня для тебя плохие новости.
	AI_Output(other,self,"DIA_Canthar_KillNow_01_02");	//Ты скоро умрешь!
	AI_Output(self,other,"DIA_Canthar_KillNow_01_03");	//ЧТО?! (смеется) С чего это ты решил?
	AI_Output(self,other,"DIA_Canthar_KillNow_01_05");	//Ты что, научился предсказывать будущее? (смеется)
	AI_Output(other,self,"DIA_Canthar_KillNow_01_12");	//Шутки кончились, Кантар! Слишком много людей хотят твоей смерти.
	AI_Output(self,other,"DIA_Canthar_KillNow_01_15");	//Черт! Что это значит?! (в панике) Стража!!!
	AI_StopProcessInfos(self);
