...
func void DIA_Akil_Soeldner_Info()
{
	...
	{
		AI_Output(self,other,"DIA_Akil_Soeldner_13_02");	//Ты этого не знаешь? Их нанял Онар. Они охраняют его ферму и собирают ренту для него.
...
func void DIA_Akil_Lieferung_Info()
{
	...
	B_LogEntry_Quiet(TOPIC_Nagur,"Я получил посылку. Теперь я могу отнести ее Нагуру...");
};
...
func void DIA_Akil_Perm_Info()
{
	...
	else
	{
		...
		{
			AI_Output(self,other,"DIA_Akil_Perm_13_08");	//Местность просто кишит зверьем! Я бы на твоем месте был поосторожнее...
		};
	};
};
...
func void DIA_Akil_SCHAFDIEBEPLATT_Info()
{
	...
	{
		AI_Output(self,other,"DIA_Akil_SCHAFDIEBEPLATT_13_03A");	//Спасибо тебе, служитель Аданоса.
	}
	else if(hero.guild == GIL_KDM)
	{
		AI_Output(self,other,"DIA_Akil_SCHAFDIEBEPLATT_13_03B");	//Хотя это и странно звучит, но я благодарю тебя, темный маг.
	}
	else if((other.guild == GIL_SEK) || (other.guild == GIL_TPL) || (other.guild == GIL_GUR))
	{
		AI_Output(self,other,"DIA_Akil_SCHAFDIEBEPLATT_13_0C");	//Спасибо тебе. Хоть вы, сектанты, и странные люди, но сострадание к чужим бедам вам не чуждо.
	}
	else
	{
...
instance DIA_Akil_Werewolf(C_Info)
{
	...
	permanent = FALSE;
	description = "По поводу недавнего нападения...";
};
...
{
	B_GivePlayerXP(100);
	AI_Output(other,self,"DIA_Akil_Werewolf_01_00");	//Что ты можешь рассказать о недавнем нападении?
	AI_Output(self,other,"DIA_Akil_Werewolf_01_01");	//Эх... Ты об этом ужасном случае.
	AI_Output(self,other,"DIA_Akil_Werewolf_01_03");	//Один из моих крестьян отправился в таверну Орлана, чтобы перед сном промочить горло. 
	AI_Output(self,other,"DIA_Akil_Werewolf_01_04");	//Как раз в тот момент я собирался пойти в дом, как вдруг у меня за спиной раздался отчаянный крик и рев зверя.
	AI_Output(self,other,"DIA_Akil_Werewolf_01_05");	//Я обернулся посмотреть, в чем дело. Но толком мне ничего разглядеть не удалось. Дело-то было почти ночью!
	AI_Output(self,other,"DIA_Akil_Werewolf_01_06");	//Тогда я со всех ног кинулся в дом, чтобы позвать на помощь остальных.
	AI_Output(self,other,"DIA_Akil_Werewolf_01_07");	//Но когда мы вернулись - ни зверя, ни крестьянина нигде не оказалось.
	AI_Output(self,other,"DIA_Akil_Werewolf_01_08");	//Мы нашли лишь клок разорванной ткани и лужу крови... Вот так.
	AI_Output(other,self,"DIA_Akil_Werewolf_01_09");	//А где вы их нашли?
	AI_Output(self,other,"DIA_Akil_Werewolf_01_10");	//Тут, прямо под мостом. Но на твоем месте я бы туда не ходил. А то мало ли что...
	B_LogEntry(TOPIC_WolfAndMan,"Акил рассказал, что на одного из крестьян напал огромный зверь. Это произошло недалеко от таверны, под мостом.");
};