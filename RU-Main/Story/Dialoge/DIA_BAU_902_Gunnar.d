...
func void DIA_Gunnar_Hallo_Info()
{
	...
	{
		AI_Output(self,other,"DIA_Gunnar_Hallo_10_06");	//Тут не рады людям из города.
	};
	if((other.guild == GIL_SEK) || (other.guild == GIL_TPL) || (other.guild == GIL_GUR))
	{
		AI_Output(self,other,"DIA_Gunnar_Hallo_10_07");	//Ты один из этих безумных сектантов? Хоть проблем от вас никаких... хотя и пользы тоже.
	};
	if((other.guild == GIL_NDW) || (other.guild == GIL_KDW))
	{
		AI_Output(self,other,"DIA_Gunnar_Hallo_10_08");	//Аданос с тобой! Все хорошо - спасибо, что поинтересовался.
	};
	if((other.guild == GIL_NDM) || (other.guild == GIL_KDM))
	{
		AI_Output(self,other,"DIA_Gunnar_Hallo_10_09");	//Ты некромант, да?!
	};
