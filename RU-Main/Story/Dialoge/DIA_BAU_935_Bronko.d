...
func void DIA_Bronko_HALLO_vergisses()
{
	...
	else
	{
		...
		{
			AI_Output(self,other,"DIA_Bronko_HALLO_vergisses_06_04");	//Что, у послушников нет денег или как? Неважно, все равно заплатишь!
		};
		if((other.guild == GIL_SEK) || (other.guild == GIL_TPL) || (other.guild == GIL_GUR))
		{
			AI_Output(self,other,"DIA_Bronko_HALLO_vergisses_06_05");	//Эй, я не знаю, кто ты и откуда взялся, но ты все равно заплатишь.
		};
	};
	Info_ClearChoices(DIA_Bronko_HALLO);
	Info_AddChoice(DIA_Bronko_HALLO,"Похоже, у меня нет выбора - вот твои деньги.",DIA_Bronko_HALLO_hiergeld);
