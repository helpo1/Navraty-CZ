...
func void DIA_Moe_Hallo_Gehen()
{
	...
	AI_Output(self,other,"DIA_Moe_Hallo_Gehen_01_02");	//Так как ты здесь новичок, я хочу сделать тебе заманчивое предложение. Ты даешь мне полсотни золотых монет и можешь идти дальше.
	...
	Info_AddChoice(DIA_Moe_Hallo,"Забудь об этом, ты не получишь ни гроша!",DIA_Moe_Hallo_Vergisses);
...
{
	AI_Output(other,self,"DIA_Moe_Hallo_Witz_15_00");	//Ох, так это портовый трактир. А я то принял его за дворец короля.
...
func void DIA_Moe_Hallo_Zahlen()
{
	...
	{
		AI_Output(other,self,"DIA_Moe_Hallo_Zahlen_15_04");	//Но у меня нет даже и десяти монет.
...
{
	AI_Output(other,self,"DIA_Moe_Hallo_Vergisses_15_00");	//Забудь об этом, ты не получишь ни гроша!
...
func void DIA_Moe_Hallo_Alles()
{
	...
	AI_Output(self,other,"DIA_Moe_Hallo_Alles_01_01");	//Отлично! Этого достаточно. Я сегодня великодушен.
	AI_StopProcessInfos(self);
};
...
instance DIA_MOE_MOEBORED(C_Info)
{
	...
	permanent = FALSE;
	description = "Кардиф жалуется на тебя!";
};
...
{
	AI_Output(other,self,"DIA_Moe_MoeBored_01_00");	//Кардиф жалуется на тебя!
	AI_Output(self,other,"DIA_Moe_MoeBored_01_01");	//Правда? (ухмыляется) И что же он говорит?
	AI_Output(other,self,"DIA_Moe_MoeBored_01_02");	//Он сказал, что ты постоянно вымогаешь деньги у людей, которые заходят в его таверну.
	AI_Output(other,self,"DIA_Moe_MoeBored_01_03");	//Это довольно плохо сказывается на его кошельке.
	AI_Output(other,self,"DIA_Moe_MoeBored_01_04");	//Люди боятся приходить сюда!
	AI_Output(self,other,"DIA_Moe_MoeBored_01_05");	//(грубо) Ну и что? Мне-то какое дело до этого!
	if(MOEISBEATEN == TRUE)
	{
		B_GivePlayerXP(100);
		AI_Output(other,self,"DIA_Moe_MoeBored_01_07");	//Думаю, тебе следует пойти в другое место.
		AI_Output(other,self,"DIA_Moe_MoeBored_01_08");	//Иначе мне придется преподать тебе урок хороших манер.
		AI_Output(self,other,"DIA_Moe_MoeBored_01_11");	//Ладно, ладно...(боязно) Только не горячись, парень! Я все понял.
		AI_Output(self,other,"DIA_Moe_MoeBored_01_12");	//Если для тебя это действительно так важно, так уж и быть, я найду себе другое местечко.
		AI_Output(self,other,"DIA_Moe_MoeBored_01_13");	//Только не бей меня больше!
		B_LogEntry(TOPIC_MOEBORED,"Я поговорил с Мо и он обещал, что больше не будет доставлять беспокойство клиентам Кардифа.");
	...
	{
		B_GivePlayerXP(50);
		AI_Output(other,self,"DIA_Moe_MoeBored_01_17");	//Думаю, тебе следует пойти в другое место.
		AI_Output(other,self,"DIA_Moe_MoeBored_01_18");	//Иначе мне придется преподать тебе урок хороших манер.
		AI_Output(self,other,"DIA_Moe_MoeBored_01_19");	//Что? (смеется) Ты мне еще и угрожать вздумал?!
		AI_Output(self,other,"DIA_Moe_MoeBored_01_20");	//Ну ладно, держись, сосунок! Сейчас я тебе покажу, как со мной связываться.
		B_LogEntry(TOPIC_MOEBORED,"Как я и ожидал, мне придется научить Мо хорошим манерам.");
		MOEATTACKME = TRUE;
		AI_StopProcessInfos(self);
		B_Attack(self,other,AR_NONE,1);
	};
...
instance DIA_MOE_MOEBOREDDONE(C_Info)
{
	...
	permanent = FALSE;
	description = "Ну что, тебе этого хватит?!";
};
...
{
	B_GivePlayerXP(150);
	AI_Output(other,self,"DIA_Moe_MoeBoredDone_01_01");	//Ну что, тебе этого хватит?!
	AI_Output(self,other,"DIA_Moe_MoeBoredDone_01_02");	//Ладно, ладно...(боязно) Не горячись, парень! Я все понял.
	AI_Output(self,other,"DIA_Moe_MoeBoredDone_01_03");	//Если для тебя это действительно так важно, так уж и быть, я найду себе другое местечко.
	AI_Output(self,other,"DIA_Moe_MoeBoredDone_01_04");	//Только не бей меня больше!
	B_LogEntry(TOPIC_MOEBORED,"Мо пообещал, что больше не будет достовлять беспокойство клиентам Кардифа.");
	MOEISAWAYFROMTAVERNE = TRUE;
	AI_StopProcessInfos(self);
	Npc_ExchangeRoutine(self,"AwayFromTaverne");
};
...
instance DIA_MOE_MOEBOREDFAIL(C_Info)
{
	...
	permanent = FALSE;
	description = "Эй ты! Мы еще не закончили.";
};
...
{
	AI_Output(other,self,"DIA_Moe_MoeBoredFail_01_01");	//Эй ты! Мы еще не закончили.
	AI_Output(self,other,"DIA_Moe_MoeBoredFail_01_02");	//Отвали, сосунок! (смеется) Все, что хотел, ты уже доказал!
	AI_Output(self,other,"DIA_Moe_MoeBoredFail_01_03");	//Даже женщины деруться лучше тебя!
	AI_Output(self,other,"DIA_Moe_MoeBoredFail_01_05");	//Проваливай...
	AI_StopProcessInfos(self);
};
