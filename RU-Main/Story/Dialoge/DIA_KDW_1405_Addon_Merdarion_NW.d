...
func void DIA_Addon_Merdarion_Aufgabe_Info()
{
	...
	AI_Output(self,other,"DIA_Addon_Merdarion_Aufgabe_06_04");	//И я даже, пожалуй...(задумываясь) никогда не слышал и не читал про нее...
...
func void DIA_Addon_Merdarion_TeleportsteinSuccess_Info()
{
	...
	AI_Output(self,other,"DIA_Addon_Merdarion_TeleportsteinSuccess_06_03");	//Невероятно...(впечатленно) Мне нужно будет поближе взглянуть на них.
};
...
{
	AI_Output(other,self,"DIA_Addon_Merdarion_PrayForGomez_01_01");	//Мне нужно благословение Аданоса.
	AI_Output(self,other,"DIA_Addon_Merdarion_PrayForGomez_01_02");	//Поговори об этом с Сатурасом.
	AI_Output(self,other,"DIA_Addon_Merdarion_PrayForGomez_01_03");	//Думаю, он сможет помочь тебе.
...
{
	B_GivePlayerXP(200);
	AI_Output(other,self,"DIA_Addon_Merdarion_RuneMagicNotWork_01_00");	//Ты все еще можешь использовать свою рунную магию?
	AI_Output(self,other,"DIA_Addon_Merdarion_RuneMagicNotWork_01_01");	//К сожалению, магия покинула мои рунические камни, они превратились в бесполезные булыжники.
	AI_Output(self,other,"DIA_Addon_Merdarion_RuneMagicNotWork_01_02");	//А все мы, маги Воды, отныне есть не что иное, как обуза для воинов, в клинках которых теперь и заключена наша надежда на спасение.
	AI_Output(self,other,"DIA_Addon_Merdarion_RuneMagicNotWork_01_03");	//Стыдно признаться, но я не в силах даже выдвинуть гипотезу, мало-мальски достоверно описывающую источник постигшей нас беды. Одно чутье лишь шепчет, что мы имеем дело с вероломством...
