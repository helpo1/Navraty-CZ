...
func void DIA_Addon_Bones_Hello_Info()
{
	...
	AI_Output(self,other,"DIA_Addon_Bones_Work_01_01");	//Я готовлюсь к новому заданию, которое дал мне Грэг.
...
func void DIA_Addon_Bones_Francis_Info()
{
	...
	AI_Output(self,other,"DIA_Addon_Bones_Francis_01_05");	//При Грэге такого не бывает. Если ты бездельничаешь, ты получаешь по шее. Вот и все.
};
...
func void DIA_Addon_Bones_WantArmor_Info()
{
	...
	AI_Output(self,other,"DIA_Addon_Bones_WantArmor_01_01");	//Я еще не свихнулся. Грэг мне голову оторвет.
	AI_Output(self,other,"DIA_Addon_Bones_WantArmor_01_02");	//Он сказал мне, что без его приказа я не должен никому их отдавать.

	if(GregIsBack == TRUE)
	{
		AI_Output(self,other,"DIA_Addon_Bones_WantArmor_01_03");	//Нет, я не могу дать их тебе. Тем более когда Грэга здесь нет.
	};
	B_LogEntry(TOPIC_Addon_BDTRuestung,"Бонес не отдаст мне доспехи, пока я не получу разрешение Грэга.");
};
...
instance DIA_Addon_Bones_GiveArmor(C_Info)
{
	...
	permanent = FALSE;
	description = "Ты должен отдать мне доспехи бандитов. Приказ Грэга.";
};
...
{
	AI_Output(other,self,"DIA_Addon_Bones_GiveArmor_15_00");	//Ты должен отдать мне доспехи бандитов. Приказ Грэга.
	AI_Output(self,other,"DIA_Addon_Bones_GiveArmor_01_01");	//Приказ Грэга? Фу, а я-то думал мне действительно придется идти на это задание.
	AI_Output(self,other,"DIA_Addon_Bones_GiveArmor_01_02");	//Разведка в лагере бандитов - это просто самоубийство.
	AI_Output(self,other,"DIA_Addon_Bones_GiveArmor_01_03");	//Пусть уж лучше Грэг взвалит на меня какую-нибудь скучную работу...
	...
	B_LogEntry(TOPIC_Addon_BDTRuestung,"Приказ Грэга возымел свое действие. Доспехи у меня!");
