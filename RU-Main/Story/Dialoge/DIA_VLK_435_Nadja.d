...
instance DIA_Nadja_Poppen(C_Info)
{
	...
	permanent = TRUE;
	description = "Иди ко мне!...(развлечься)";
};
...
func void DIA_Nadja_Poppen_Info()
{
	...
	AI_Output(other,self,"DIA_Nadja_Poppen_16_99");	//Иди ко мне!
...
func void DIA_Nadja_BUYHERB_Info()
{
	...
	if(Npc_GetDistToWP(self,"NW_CITY_HABOUR_PUFF_NADJA") < 500)
	{
		...
		else
		{
			...
			AI_Output(self,other,"DIA_Nadja_BUYHERB_16_04");	//Полсотни золотых будет достаточно.
			Nadja_Money = TRUE;
		};
	}
	else
...
instance DIA_Nadja_WANT_HERB(C_Info)
{
	...
	permanent = FALSE;
	description = "А теперь скажи мне, где можно купить травки?";
};
