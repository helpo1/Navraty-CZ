...
func void DIA_Sekob_KohleRaus_Info()
{
	...
	AI_Output(self,other,"DIA_Sekob_KohleRaus_01_01");	//Ты не получишь ни монеты из моего кошелька, преступник. ТОЛЬКО НЕ ТЫ!
	AI_StopProcessInfos(self);
...
func void DIA_Sekob_Again_Info()
{
	...
	{
		Info_AddChoice(DIA_Sekob_Again,"Это мое...",DIA_Sekob_Again_verarscht);
	}
	else
	{
...
func void DIA_Sekob_DMT_Info()
{
	...
	{
		AI_Output(self,other,"DIA_Sekob_DMT_01_04A");	//Вы же, ребята из Братства, не такие черствые, как эти тупоголовые наемники или королевские псы!
	};
	if((other.guild == GIL_NDW) || (other.guild == GIL_KDW))
	{
		AI_Output(self,other,"DIA_Sekob_DMT_01_04B");	//Аданос с тобой!
	};
	if((other.guild == GIL_NDM) || (other.guild == GIL_KDM))
	{
		AI_Output(self,other,"DIA_Sekob_DMT_01_04C");	//Прошу тебя, темный маг!
	};
...
func void DIA_Sekob_DMTWEG_Info()
{
	...
	if((other.guild == GIL_SEK) || (other.guild == GIL_TPL) || (other.guild == GIL_GUR))
	{
		AI_Output(self,other,"DIA_Sekob_DMTWEG_01_04");	//Спасибо тебе, болотник. Все-таки среди вас есть достойные люди!
	};
	if((other.guild == GIL_NDW) || (other.guild == GIL_KDW))
	{
		AI_Output(self,other,"DIA_Sekob_DMTWEG_01_05");	//Благодарю тебя, маг. Что было бы с нами, если бы нас не защитил Аданос!.
	};
	if((other.guild == GIL_NDM) || (other.guild == GIL_KDM))
	{
		AI_Output(self,other,"DIA_Sekob_DMTWEG_01_06");	//Прими мою благодарность, некромант. Никогда не думал, что слуги Белиара способны творить добро!
	};
...
func void DIA_Sekob_BELOHNUNG_Info()
{
	...
	if((hero.guild == GIL_KDF) || (hero.guild == GIL_NDW) || (hero.guild == GIL_KDW) || (hero.guild == GIL_NDM) || (hero.guild == GIL_KDM))
	{
		...
		AI_Output(self,other,"DIA_Sekob_BELOHNUNG_01_07");	//Очень жаль. Много лет назад я поклялся молчать об этом, и у меня нет никакого желания нарушить свое обещание.
	};
	if((other.guild == GIL_SEK) || (other.guild == GIL_TPL) || (other.guild == GIL_GUR))
	{
		AI_Output(other,self,"DIA_Sekob_BELOHNUNG_01_07A");	//Конечно, мы - люди из Братства - всегда готовы прийти на помощь тем, кто нуждается в ней. Но ты же понимаешь, что любая услуга чего-то стоит в наше время...
	};
...
func void DIA_Sekob_PERM_Info()
{
	...
	else
	{
		...
		else if((Kapitel >= 5) && (MIS_bringRosiBackToSekob != LOG_SUCCESS) && (MIS_bringRosiBackToSekob != LOG_Running)) 
		{
			...
			B_LogEntry(TOPIC_bringRosiBackToSekob,"Пропала жена Секоба - Рози! Секоб хочет найти и вернуть ее назад.");
		}
		else
		{
