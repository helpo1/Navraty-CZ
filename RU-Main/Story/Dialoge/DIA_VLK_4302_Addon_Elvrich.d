...
{
	AI_Output(other,self,"DIA_Addon_Elvrich_ElvrichGritta_01_00");	//Как жизнь, Элврих?
	AI_Output(other,self,"DIA_Addon_Elvrich_ElvrichGritta_01_02");	//Все никак не можешь прийти в себя после письма от Люсии?
	AI_Output(self,other,"DIA_Addon_Elvrich_ElvrichGritta_01_03");	//Я уже смирился с этим. Думаю, мне лучше стоит забыть о ней и сосредоточиться на чем-то более реальном.
	AI_Output(other,self,"DIA_Addon_Elvrich_ElvrichGritta_01_06");	//А как ты относишься к Гритте, племяннице Торбена?
	AI_Output(self,other,"DIA_Addon_Elvrich_ElvrichGritta_01_07");	//Ну...(смущенно) ну, она очень милая! Я бы даже сказал, что...
	AI_Output(other,self,"DIA_Addon_Elvrich_ElvrichGritta_01_08");	//Так что тебе мешает сказать ей о своих чувствах?
	AI_Output(self,other,"DIA_Addon_Elvrich_ElvrichGritta_01_09");	//Да ты что - с ума сошел? (испуганно) Нет, парень, этого я делать не буду!
	AI_Output(other,self,"DIA_Addon_Elvrich_ElvrichGritta_01_14");	//Ну, тогда ты просидишь здесь всю свою жизнь ОДИН!
	AI_Output(self,other,"DIA_Addon_Elvrich_ElvrichGritta_01_19");	//Эх... возможно, ты и прав! Мне уже действительно пора поговорить с ней.
	AI_Output(other,self,"DIA_Addon_Elvrich_ElvrichGritta_01_26");	//Тогда пожелаю тебе удачи.
	B_LogEntry(TOPIC_ELVRIHMARRYGRITTA,"Я смог убедить Элвриха, чтобы он поговорил с Гриттой.");
...
instance DIA_ADDON_ELVRICH_ELVRICHGRITTADONE(C_Info)
{
	...
	permanent = FALSE;
	description = "Поговорил с Гриттой?";
};
...
{
	AI_Output(other,self,"DIA_Addon_Elvrich_ElvrichGrittaDone_01_00");	//Поговорил с Гриттой?
	AI_Output(self,other,"DIA_Addon_Elvrich_ElvrichGrittaDone_01_01");	//Да! (восторженно) Спасибо тебе за совет! Теперь я счастлив!
	AI_Output(other,self,"DIA_Addon_Elvrich_ElvrichGrittaDone_01_02");	//Ну, тогда я рад за вас.
	AI_Output(self,other,"DIA_Addon_Elvrich_ElvrichGrittaDone_01_03");	//Спасибо!
	AI_Output(self,other,"DIA_Addon_Elvrich_ElvrichGrittaDone_01_04");	//Я никогда не забуду, что ты для меня сделал, и поэтому хочу отблагодарить тебя!
	AI_Output(self,other,"DIA_Addon_Elvrich_ElvrichGrittaDone_01_05");	//Если тебе когда-нибудь нужны будут новые отмычки, то можешь смело обращаться ко мне.
	AI_Output(self,other,"DIA_Addon_Elvrich_ElvrichGrittaDone_01_06");	//Я всегда тебе дам пару штук, и причем совершенно бесплатно!
...
instance DIA_ADDON_ELVRICH_DAILYRATION(C_Info)
{
	...
	permanent = TRUE;
	description = "Как насчет пары отмычек.";
};
...
func void dia_addon_elvrich_dailyration_info()
{
	...
	AI_Output(other,self,"DIA_Addon_Elvrich_DailyRation_01_01");	//Как насчет пары отмычек?
	AI_Output(self,other,"DIA_Addon_Elvrich_DailyRation_01_02");	//Конечно! Вот, возьми...
