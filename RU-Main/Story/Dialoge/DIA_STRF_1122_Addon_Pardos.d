...
func void DIA_Addon_Pardos_Hi_Info()
{
	...
	AI_Output(self,other,"DIA_Addon_Pardos_Hi_03_01");	//Я устал...(устало) Я совершенно изможден! Скажи, сейчас солнечный день или ясная ночь, и тысячи звезд мерцают на небосклоне?
	AI_Output(self,other,"DIA_Addon_Pardos_Hi_03_02");	//Я не знаю, идет ли теплый летний дождь или дует прохладный вечерний ветер. Я больше ничего не чувствую!
	AI_Output(self,other,"DIA_Addon_Pardos_Hi_03_03");	//Первыми здесь отказывают глаза.
};
...
func void DIA_Addon_Pardos_trank_Info()
{
	...
	{
		Info_AddChoice(DIA_Addon_Pardos_trank,"Дать эссенцию лечения",DIA_Addon_Pardos_trank_01);
	};
	if(Npc_HasItems(other,ItPo_Health_02))
	{
		Info_AddChoice(DIA_Addon_Pardos_trank,"дать экстракт лечения",DIA_Addon_Pardos_trank_02);
	};
	if(Npc_HasItems(other,ItPo_Health_03))
	{
		Info_AddChoice(DIA_Addon_Pardos_trank,"Дать эликсир лечения",DIA_Addon_Pardos_trank_03);
	};
	if(Npc_HasItems(other,ItFo_Addon_Meatsoup))
	{
		Info_AddChoice(DIA_Addon_Pardos_trank,"Дать мясной суп.",DIA_Addon_Pardos_trank_Soup);
	};
};
...
{
	AI_Output(other,self,"DIA_Addon_Pardos_trank_soup_15_00");	//Вот, поешь мясного супчика.
