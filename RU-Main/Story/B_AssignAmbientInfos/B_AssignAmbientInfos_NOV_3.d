...
func void DIA_NOV_3_Fegen_Info()
{
	...
	if(Hlp_GetInstanceID(Feger2) == Hlp_GetInstanceID(self))
	{
		if(Feger2_Permanent == FALSE)
		{
			...
			AI_Output(self,other,"DIA_NOV_3_Fegen_03_09");	//Я прошу всего полсотни золотых монет, мне нужно заплатить их Парлану.
			B_LogEntry(Topic_ParlanFegen,"Послушник у церкви поможет мне, если я дам ему полсотни золотых монет.");
...
func void DIA_NOV_3_STANDARD_Info()
{
	...
	if(Kapitel == 5)
	{
		AI_Output(self,other,"DIA_NOV_3_STANDARD_03_12");	//Хвала Инносу, нового кризиса нет. Мы должны дальше идти дорогой нашего господина, потому что только с его помощью мы сможем противостоять злу.
	};
	if(Kapitel >= 6)
	{
		AI_Output(self,other,"DIA_NOV_3_STANDARD_03_13");	//А ты сам не знаешь?! Орки, орки, орки - вот единственная новость за последнее время. И с каждым днем их появляется еще больше!
	};
};
