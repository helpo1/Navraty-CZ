...
func void PC_PrayIdol_Sacred_Info()
{
	...
	if(hero.guild == GIL_KDM)
	{
		...
		{
			Info_AddChoice(PC_PrayIdol_Sacred,"...100 золотых монет (+ 2 к карме Белиара)",PC_PrayIdol_Sacred_SmallPay);
		};
		if(Npc_HasItems(hero,ItMi_Gold) >= 500)
		{
			Info_AddChoice(PC_PrayIdol_Sacred,"...500 золотых монет (+ 10 к карме Белиара)",PC_PrayIdol_Sacred_MediumPay);
		};
		if(Npc_HasItems(hero,ItMi_Gold) >= 1000)
		{
			Info_AddChoice(PC_PrayIdol_Sacred,"...1000 золотых монет (+ 20 к карме Белиара)",PC_PrayIdol_Sacred_BigPay);
		};
	}
	...
	{	
		if(Npc_HasItems(hero,ItMi_Gold) >= 500)
		{
			Info_AddChoice(PC_PrayIdol_Sacred,"...500 золотых монет (+ 1 к карме Белиара)",PC_PrayIdol_Sacred_MediumPay);
		};
		if(Npc_HasItems(hero,ItMi_Gold) >= 1000)
		{
			Info_AddChoice(PC_PrayIdol_Sacred,"...1000 золотых монет (+ 5 к карме Белиара, - 5 к карме Инноса)",PC_PrayIdol_Sacred_BigPay);
		};
	}
	else
	{
		if(Npc_HasItems(hero,ItMi_Gold) >= 100)
		{
			Info_AddChoice(PC_PrayIdol_Sacred,"...100 золотых монет (+ 1 к карме Белиара)",PC_PrayIdol_Sacred_SmallPay);
		};
		if(Npc_HasItems(hero,ItMi_Gold) >= 500)
		{
			Info_AddChoice(PC_PrayIdol_Sacred,"...500 золотых монет (+ 5 к карме Белиара)",PC_PrayIdol_Sacred_MediumPay);
		};
		if(Npc_HasItems(hero,ItMi_Gold) >= 1000)
		{
			Info_AddChoice(PC_PrayIdol_Sacred,"...1000 золотых монет (+ 10 к карме Белиара)",PC_PrayIdol_Sacred_BigPay);
		};
	};
};
...
func void PC_PrayIdol_RaiseAttribute_Info()
{
	...
	if(TempMaxUpGod > MaxUpGod)
	{
		...
		Info_AddChoice(PC_PrayIdol_RaiseAttribute,"Даруй мне силу (Сила: + 5, Требуется кармы: 100)",PC_PrayIdol_RaiseAttribute_STR);
		Info_AddChoice(PC_PrayIdol_RaiseAttribute,"Даруй мне ловкость (Ловкость: + 10, Требуется кармы: 100)",PC_PrayIdol_RaiseAttribute_DEX);
		Info_AddChoice(PC_PrayIdol_RaiseAttribute,"Даруй мне магическую силу (Мана: + 15, Требуется кармы: 100)",PC_PrayIdol_RaiseAttribute_MANA);
		Info_AddChoice(PC_PrayIdol_RaiseAttribute,"Даруй мне жизненную силу (Здоровье: + 40, Требуется кармы: 100)",PC_PrayIdol_RaiseAttribute_HP);
		Info_AddChoice(PC_PrayIdol_RaiseAttribute,"Даруй мне мудрость (Интеллект: + 5, Требуется кармы: 50)",PC_PrayIdol_RaiseAttribute_INT);

		if(ATR_STAMINA_MAX[0] <= 90)
		{
			Info_AddChoice(PC_PrayIdol_RaiseAttribute,"Даруй мне выносливость (Выносливость: + 10, Требуется кармы: 50)",PC_PrayIdol_RaiseAttribute_STM);
		};
	}
	else
	{
...
instance PC_PRAYRITUALSTONE_AZGOLOR(C_Info)
{
	...
	permanent = FALSE;
	description = "Провести ритуал вызова";
};
...
instance PC_PRAYIDOL_TRIRAMAR(C_Info)
{
	...
	permanent = FALSE;
	description = "Просить Мора Уларту";
};
...
instance PC_PRAYIDOL_UPGRATEBELIARSWEAPON(C_Info)
{
	...
	permanent = TRUE;
	description = "Улучшить 'Коготь Белиара' (требуются души)";
};
...
instance PC_PRAYIDOL_GETBELIARSUPERWEAPON(C_Info)
{
	...
	permanent = TRUE;
	description = "Перенести душу С'эньяка в 'Коготь Белиара'";
};
...
instance PC_PRAYIDOL_INVOKESOULSSTONEINSBW(C_Info)
{
	...
	permanent = TRUE;
	description = "Даровать души Темному Богу";
};
...
instance PC_PRAYIDOL_INVOKESOULSSTONEINSBW_SENYAK(C_Info)
{
	...
	permanent = TRUE;
	description = "Даровать душу С'эньяка";
};
...
func void pc_prayidol_prayforpower_info()
{
	if(BELIARSAYHISWILL == TRUE)
	{
		if(ORCLEADERISDEAD == TRUE)
		{
			...
			B_LogEntry(TOPIC_GUARDIANS,"Я получил поддержку Белиара.");
			AI_Wait(hero,1);
	...
	else
	{
		...
		if(ORCLEADERISDEAD == TRUE)
		{
			B_LogEntry(TOPIC_BELIARWILL,"Белиар приказал мне уничтожить главаря орков! По всей видимости, речь идет об Ур-Тралле... Но вождь орков уже мертв! Так что думаю просто стоит попросить Белиара о помощи еще раз.");
			AI_Wait(hero,1);
		}
		else
		{
			B_LogEntry(TOPIC_BELIARWILL,"Белиар приказал мне уничтожить главаря орков! По всей видимости, речь идет об Ур-Тралле...");
			AI_Wait(hero,1);
		};
	};
...
instance PC_PRAYIDOL_PRAYFORHELP(C_Info)
{
	...
	permanent = TRUE;
	description = "Молиться и просить о помощи";
};
...
func void pc_prayidol_prayforhelp_info()
{
	...
	if((BELIARPRAYCOUNT > 0) && (hero.guild != GIL_PAL) && (hero.guild != GIL_KDF))
	{
		...
		Wld_PlayEffect("FX_EarthQuake",self,self,0,0,0,FALSE);
		AI_Wait(self,10);
		Log_AddEntry(TOPIC_GOLDDRAGONPORTAL,"Кажется, Белиар услышал мою молитву!");
		AI_Print("Кажется, Белиар услышал мою молитву!");
	}
	else
	{
		AI_Wait(self,10);
...
instance PC_PRAYIDOL_PRAY_BLESSROBA(C_Info)
{
	...
	permanent = TRUE;
	description = "Просить об освящении робы";
};
...
func void PC_PRAYIDOL_PRAY_BLESSROBA_info()
{
	...
	{
		Info_AddChoice(PC_PRAYIDOL_PRAY_BLESSROBA,"Принести в дар душу С'эньяка",PC_PRAYIDOL_PRAY_BLESSROBA_Senyak);
	};
	if(Npc_HasItems(hero,itmi_stonesoul) >= 100)
	{
		Info_AddChoice(PC_PRAYIDOL_PRAY_BLESSROBA,"Принести в дар обычные души",PC_PRAYIDOL_PRAY_BLESSROBA_Common);
	};
};
...
func void PC_PRAYIDOL_PRAY_BLESSROBA_Senyak()
{
 	if(BELIARPRAYCOUNT >= 200)
	{
		...
		AI_Print("Темный Бог освятил вашу робу!");
		Wld_PlayEffect("spellFX_INCOVATION_VIOLET",self,self,0,0,0,FALSE);
		Wld_PlayEffect("SPELLFX_BELIARSHRINE",self,self,0,0,0,FALSE);
		Wld_PlayEffect("FX_EarthQuake",self,self,0,0,0,FALSE);
	}
...
instance PC_PRAYIDOL_PRAY_HASIM(C_Info)
{
	...
	permanent = TRUE;
	description = "Помолиться за душу Хасима";
};
...
instance PC_PRAYIDOL_HIDE_Open(C_Info)
{
	...
	permanent = FALSE;
	description = "(прочесть свиток Масиаф)";
};
...
func void PC_PRAYIDOL_HIDE_Open_Info()
{
	...
	MasiafSecret = TRUE;
	B_LogEntry(TOPIC_MasiafStory,"Я прочитал свиток перед алтарем Белиара и мне открылся потайной проход.");
};
...
instance PC_PRAYIDOL_HANIARTRIBE(C_Info)
{
	...
	permanent = TRUE;
	description = "Поднести золото Ханиара";
};
...
func void PC_PRAYIDOL_HANIARTRIBE_Info()
{
	...
	concatText = "Темный Бог принял ваш дар...";
	AI_Print(concatText);
	...
	B_LogEntry(TOPIC_BeliarTribe,"Я принес дар Белиару! Уверен, что Ханиара обрадует эта новость.");
	BELIARPRAYCOUNT = BELIARPRAYCOUNT + 5;
};
...
instance PC_PRAYIDOL_SOULRIVER(C_Info)
{
	...
	permanent = TRUE;
	description = "Молиться для приношения жертвенного дара";
};
...
instance PC_PRAYIDOL_SOULRIVER_BLESS(C_Info)
{
	...
	permanent = TRUE;
	description = "(окропить алтарь Вытяжкой Душ)";
};
