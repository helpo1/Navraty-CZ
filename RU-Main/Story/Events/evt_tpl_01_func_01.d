...
func void event_alef_found_func()
{
	if((MIS_VIPERNUGGETS == LOG_Running) && (ALEF_FOUND_DONE == FALSE))
	{
		if(Npc_HasItems(hero,itat_skeletonbonealef) >= 1)
		{
			if(Kapitel >= 5)
			{
				B_LogEntry(TOPIC_VIPERNUGGETS,"Боюсь, теперь никто не узнает, где Алеф зарыл руду, поскольку тот мертв. Он унес эту тайну с собой в могилу, а я не знаю ни одного способа вернуть его с того света.");
				MIS_VIPERNUGGETS = LOG_FAILED;
			}
			else
			{
				B_LogEntry(TOPIC_VIPERNUGGETS,"Боюсь, теперь никто не узнает, где Алеф зарыл руду, поскольку тот мертв. Он унес эту тайну с собой в могилу, а я не знаю ни одного способа вернуть его с того света! Хотя... возможно, тот, кто владеет искусством некромантии мог бы это сделать?");
			};
...
func void evt_li_teleport_frdc_func()
{
	...
	else
	{
		...
		AI_Print("Магия этого места не дает вам его покинуть...");
	};
};
...
func void evt_darktower_opengate_func()
{
	if(OSTAISDEAD == TRUE)
	{
		...
		{
			ENTERASHERON = TRUE;
			B_LogEntry(TOPIC_XARDASTASKSFOUR,"Решетка больше не преграждает мой путь. Теперь надо найти Некрономикон!");
		};
	};
};
...
func void evt_gader_01_func_s1()
{
	...
	{
		B_GivePlayerXP(100);
		B_LogEntry(TOPIC_GADERTEST,"Я возложил магический камень на алтарь в храме Хранителей Мертвых.");
		GADERSTONE_ONE = TRUE;
