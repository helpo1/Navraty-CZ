...
const int LOADBAR_POSX 	= 3000;	//Положение по оси X
const int LOADBAR_POSY 	= 5000; //Положение по оси Y
const int LOADBAR_SIZEX 	= 256; //Размер по оси X
const int LOADBAR_SIZEY	= 32;  //Рзамер по оси Y
const string MENU_BACK_WORLD = "INSEL_FREE.ZEN";	//Файл фонового мира для меню
const int _bBackWorldEnabled = 1;			//Включить/выключить фоновый мир для меню
const int bDualSystem = 0;					//Двуручный бой ( 0 - выключен/ 1 - включен)
const string _2H2W_OVERLAY = "Humans_2x2ST3.MDS";	//Оверлей для двуручного боя
const int bEnableMsgCallback = 0;			//Включить функцию func void MsgCallback()
const int bRemoveIVFXAfterUse = 0;			//Включать эффекты оружия(VFX) только когда оно в руке
const int bAddAIVARS	= 1; //Добавить 20 AIVAR
const int HERO_NORMAL		= 	0;	//Нормальный режим
const int HERO_THREATENED	= 	1;	//Угроза
const int HERO_FIGHTING		= 	2;	//Режим боя
